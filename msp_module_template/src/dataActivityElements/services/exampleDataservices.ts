...
const someFixture = {
  artefacts: [
    { id: '1', name: 'Artefact 1' },
    { id: '2', name: 'Artefact 2' },
  ],
  assertions: [
    { id: '1', name: 'Assertion 1' },
    { id: '2', name: 'Assertion 2' },
  ],
};

function toModule_TemplateViewData(name: string, entityType: string, row: Record<string, unknown>) {
  return {
    namespace: 'pamela',
    name,
    version: '1.0.0',
...
export async function listModule_TemplateArtefactsHandler(
  _payload: unknown,
  resultBuilder: ServiceActivityResultBuilder,
): Promise<ServiceActivityResultBuilder> {
  const rows = someFixture.artefacts.map((artefact) =>
    toModule_TemplateViewData('Module_TemplateArtefacts', 'artefact', artefact),
  );
...
export async function listModule_TemplateAssertionsHandler(
  _payload: unknown,
  resultBuilder: ServiceActivityResultBuilder,
): Promise<ServiceActivityResultBuilder> {
  const rows = someFixture.assertions.map((assertion) =>
    toModule_TemplateViewData('Module_TemplateAssertions', 'assertion', assertion),
  );
...
export async function writeModule_TemplateAssertionsHandler(
...
export async function writeModule_TemplateSemanticAssertionsHandler(
...
export type Module_TemplateStage1DataService = ActivitySet;

import { exampleDataActivities } from '../activities/exampleDataActivities.js';

export function getDataServiceActivities(): ActivitySet {
  return exampleDataActivities;
}
