import { createSchema } from 'msp_common';

export type AwsClusterSetupRepository = {
  repositoryName: string;
  repositoryUri?: string;
  region: string;
};

export type AwsClusterSetupEksDesiredState = {
  clusterVersion?: string;
  nodeCount?: number;
};

export type AwsClusterSetupEcrDesiredState = {
  repositories?: AwsClusterSetupRepository[];
};

export type AwsClusterSetupNetworkDesiredState = {
  vpcCidr?: string;
  publicSubnetCount?: number;
  privateSubnetCount?: number;
};

export type AwsClusterSetupDesiredState = {
  eks?: AwsClusterSetupEksDesiredState;
  ecr?: AwsClusterSetupEcrDesiredState;
  network?: AwsClusterSetupNetworkDesiredState;
};

export const awsClusterSetupRepositorySchema = createSchema('awsClusterSetupRepository')
  .withFQId({ namespace: 'aws', version: '1.0'})
  .withProperty('repositoryName')
  .forType<string>()
  .withDictionaryId('aws-setup-repository-name', '1.0')
  .withInfoType('Text')
  .withDefaultLabel('Repository Name')
  .endProperty()
  .withProperty('repositoryUri')
  .forType<string>()
  .withDictionaryId('aws-setup-repository-uri', '1.0')
  .withInfoType('Text')
  .withDefaultLabel('Repository URI')
  .endProperty()
  .withProperty('region')
  .forType<string>()
  .withDictionaryId('aws-setup-repository-region', '1.0')
  .withInfoType('Text')
  .withDefaultLabel('Region')
  .endProperty()
  .buildSchema();

export const awsClusterSetupEksDesiredStateSchema = createSchema('awsClusterSetupEksDesiredState')
  .withFQId({ namespace: 'aws', version: '1.0'})
  .withProperty('clusterVersion')
  .forType<string>()
  .withDictionaryId('aws-setup-eks-cluster-version', '1.0')
  .withInfoType('Text')
  .withDefaultLabel('Cluster Version')
  .endProperty()
  .withProperty('nodeCount')
  .forType<number>()
  .withDictionaryId('aws-setup-eks-node-count', '1.0')
  .withInfoType('Integer')
  .withDefaultLabel('Node Count')
  .endProperty()
  .buildSchema();

export const awsClusterSetupEcrDesiredStateSchema = createSchema('awsClusterSetupEcrDesiredState')
  .withFQId({ namespace: 'aws', version: '1.0'})
  .withProperty('repositoryName')
    .forType<string>()
    .withDictionaryId('aws-setup-ecr-repositories-repositoryName', '1.0')
    .withInfoType('Text')
    .withDefaultLabel('Repository Name')
    .endProperty()
  .withProperty('repositoryUri')
    .forType<string>()
    .withDictionaryId('aws-setup-ecr-repositories-repositoryUri', '1.0')
    .withInfoType('Text')
    .withDefaultLabel('Repository URI')
    .endProperty()
  .withProperty('region')
    .forType<string>()
    .withDictionaryId('aws-setup-ecr-repositories-region', '1.0')
    .withInfoType('Text')
    .withDefaultLabel('Region')
    .endProperty()
  .buildSchema();

export const awsClusterSetupNetworkDesiredStateSchema = createSchema('awsClusterSetupNetworkDesiredState')
  .withFQId({ namespace: 'aws', version: '1.0'})
  .withProperty('vpcCidr')
  .forType<string>()
  .withDictionaryId('aws-setup-network-vpc-cidr', '1.0')
  .withInfoType('Text')
  .withDefaultLabel('VPC CIDR')
  .endProperty()
  .withProperty('publicSubnetCount')
  .forType<number>()
  .withDictionaryId('aws-setup-network-public-subnet-count', '1.0')
  .withInfoType('Integer')
  .withDefaultLabel('Public Subnet Count')
  .endProperty()
  .withProperty('privateSubnetCount')
  .forType<number>()
  .withDictionaryId('aws-setup-network-private-subnet-count', '1.0')
  .withInfoType('Integer')
  .withDefaultLabel('Private Subnet Count')
  .endProperty()
  .buildSchema();

export const awsClusterSetupDesiredStateSchema = createSchema('awsClusterSetupDesiredState')
  .withFQId({ namespace: 'aws', version: '1.0'})

  .buildSchema();

export const awsClusterSetupConfigSchema = createSchema('awsClusterSetupConfig')
  .withFQId({ namespace: 'aws', version: '1.0'})
  .withProperty('setupId')
  .forType<string>()
  .withDictionaryId('aws-setup-setup-id', '1.0')
  .withInfoType('Text')
  .withDefaultLabel('Setup Id')
  .endProperty()
  .withProperty('accountId')
  .forType<string>()
  .withDictionaryId('aws-setup-account-id', '1.0')
  .withInfoType('Text')
  .withDefaultLabel('Account Id')
  .endProperty()
  .withProperty('accountName')
  .forType<string>()
  .withDictionaryId('aws-setup-account-name', '1.0')
  .withInfoType('Text')
  .withDefaultLabel('Account Name')
  .endProperty()
  .withProperty('accessKeyId')
  .forType<string>()
  .withDictionaryId('aws-setup-access-key-id', '1.0')
  .withInfoType('Text')
  .withDefaultLabel('Access Key Id')
  .endProperty()
  .withProperty('environmentPurpose')
  .forType<string>()
  .withDictionaryId('aws-setup-environment-purpose', '1.0')
  .withInfoType('Text')
  .withDefaultLabel('Environment Purpose')
  .endProperty()
  .withProperty('abMode')
  .forType<boolean>()
  .withDictionaryId('aws-setup-ab-mode', '1.0')
  .withInfoType('Boolean')
  .withDefaultLabel('A/B Mode')
  .endProperty()
  .withProperty('region')
  .forType<string>()
  .withDictionaryId('aws-setup-region', '1.0')
  .withInfoType('Text')
  .withDefaultLabel('Region')
  .endProperty()
  .withProperty('clusterName')
  .forType<string>()
  .withDictionaryId('aws-setup-cluster-name', '1.0')
  .withInfoType('Text')
  .withDefaultLabel('Cluster Name')
  .endProperty()
  .withProperty('connectionStatus')
  .forType<string>()
  .withDictionaryId('aws-setup-connection-status', '1.0')
  .withInfoType('Text')
  .withDefaultLabel('Connection Status')
  .endProperty()
  .withProperty('connectionMessage')
  .forType<string>()
  .withDictionaryId('aws-setup-connection-message', '1.0')
  .withInfoType('Text')
  .withDefaultLabel('Connection Message')
  .endProperty()
  .withProperty('connectionCheckedAt')
  .forType<string>()
  .withDictionaryId('aws-setup-connection-checked-at', '1.0')
  .withInfoType('Text')
  .withDefaultLabel('Connection Checked At')
  .endProperty()
  .withProperty('wizardVersion')
  .forType<string>()
  .withDictionaryId('aws-setup-wizard-version', '1.0')
  .withInfoType('Text')
  .withDefaultLabel('Wizard Version')
  .endProperty()
  .withProperty('status')
  .forType<string>()
  .withDictionaryId('aws-setup-status', '1.0')
  .withInfoType('Text')
  .withDefaultLabel('Status')
  .endProperty()
 
  .withProperty('updatedAt')
  .forType<string>()
  .withDictionaryId('aws-setup-updated-at', '1.0')
  .withInfoType('Text')
  .withDefaultLabel('Updated At')
  .endProperty()
  .buildSchema();

export const awsDesiredResourceConfigSchema = createSchema('awsDesiredResourceConfig')
  .withFQId({ namespace: 'aws', version: '1.0'})
  .withProperty('setupCaseId')
  .forType<string>()
  .withDictionaryId('aws-desired-case-id', '1.0')
  .withInfoType('Text')
  .withDefaultLabel('Setup Case Id')
  .endProperty()
  .withProperty('setupRunId')
  .forType<string>()
  .withDictionaryId('aws-desired-run-id', '1.0')
  .withInfoType('Text')
  .withDefaultLabel('Setup Run Id')
  .endProperty()
  .withProperty('region')
  .forType<string>()
  .withDictionaryId('aws-desired-region', '1.0')
  .withInfoType('Text')
  .withDefaultLabel('Region')
  .endProperty()
  .withProperty('resources')
  .forType<any>()
  .withDictionaryId('aws-desired-resources', '1.0')
  .withInfoType('Text')
  .withDefaultLabel('Resources')
  .endProperty()
  .withProperty('updatedAt')
  .forType<string>()
  .withDictionaryId('aws-desired-updated-at', '1.0')
  .withInfoType('Text')
  .withDefaultLabel('Updated At')
  .endProperty()
  .buildSchema();

export const ecrSetupConfigSchema = createSchema('ecrSetupConfig')
  .withFQId({ namespace: 'aws', version: '1.0'})
  .withProperty('setupId')
  .forType<string>()
  .withDictionaryId('ecr-setup-id', '1.0')
  .withInfoType('Text')
  .withDefaultLabel('Setup Id')
  .endProperty()
  .withProperty('accountId')
  .forType<string>()
  .withDictionaryId('ecr-account-id', '1.0')
  .withInfoType('Text')
  .withDefaultLabel('Account Id')
  .endProperty()
  .withProperty('accountName')
  .forType<string>()
  .withDictionaryId('ecr-account-name', '1.0')
  .withInfoType('Text')
  .withDefaultLabel('Account Name')
  .endProperty()
  .withProperty('region')
  .forType<string>()
  .withDictionaryId('ecr-region', '1.0')
  .withInfoType('Text')
  .withDefaultLabel('Region')
  .endProperty()
  .withProperty('wizardVersion')
  .forType<string>()
  .withDictionaryId('ecr-wizard-version', '1.0')
  .withInfoType('Text')
  .withDefaultLabel('Wizard Version')
  .endProperty()
  .withProperty('status')
  .forType<string>()
  .withDictionaryId('ecr-status', '1.0')
  .withInfoType('Text')
  .withDefaultLabel('Status')
  .endProperty()
  .withProperty('desiredState')
  .forType<any>()
  .withDictionaryId('ecr-desired-state', '1.0')
  .withInfoType('Text')
  .withDefaultLabel('Desired State')
  .endProperty()
  .withProperty('updatedAt')
  .forType<string>()
  .withDictionaryId('ecr-updated-at', '1.0')
  .withInfoType('Text')
  .withDefaultLabel('Updated At')
  .endProperty()
  .buildSchema();
