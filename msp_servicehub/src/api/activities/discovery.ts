...
function getDataFeatures(source: any): any[] {
    const explicit = Array.isArray(source?.dataFeatures) ? source.dataFeatures : [];
    const legacy = Array.isArray(source?.informationPackages) ? source.informationPackages : [];
