...
app.use(_static(join(_dirname, "public")));
app.use(fileUpload({
  limits: {
...
const corsoptions = {
  ...
  // some legacy browsers (IE11, various SmartTVs) chol
};
...
app.use("/api/v1", async (req, res, next) => {
  ...
  try {
    ...
    let queryParams = Object.entries(req.query ?? {}).map(([key, value]) => `${key}=${encodeURIComponent(value)}`).join("&");
    ...
    const stream = (_a = upstreamRes.body) == null ? void 0 : _a.pipe(res);
    (_b = upstreamRes.body) == null ? void 0 : _b.on("error", (err) => {
      console.error("Error in upstream response stream:", err);
      controller.abort();
      res.end();
    });
    stream == null ? void 0 : stream.on("error", (err) => {
    ...
    req.socket.on("close", () => {
      if (!res.writableEnded) {
        ...
        stream == null ? void 0 : stream.destroy();
        controller.abort();
      }
    });
