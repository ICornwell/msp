...
export function reactExportCode(): string {
  return `
    ...
    const sharedModule = pick('createContext')
      ? candidates.find((c) => c && c.createContext)
      : (moduleFactory?.default?.default?.default ?? moduleFactory?.default?.default ?? moduleFactory?.default ?? moduleFactory);
