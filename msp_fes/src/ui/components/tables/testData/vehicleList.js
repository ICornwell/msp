...
export const vehicleRollupAggregationMeta = {
    type: AggregationType.None,
    use: AggregationType.None,
...
export const vehicleRollupLabels = {
    type: 'Type',
    use: 'Use',
    vehicleCount: 'Count',
...
function createVehicle(id, reg, type, use, make, model, year, value, basePremium, factors // modifiers for each factor
) {
    const ratingFactors = {
...
function randomModifier(base, variance) {
    return Math.round((base + (rng.next() - 0.5) * variance) * 100) / 100;
}
function generateVehicles(count, seed) {
    // Reset RNG for reproducible results
    rng.reset(seed ?? DEFAULT_SEED);
    const vehicles = [];
    const types = ['car', 'car', 'car', 'van', 'van', 'truck', 'motorcycle', 'bus'];
    const uses = ['private', 'private', 'business', 'commercial', 'fleet'];
    for (let i = 1; i <= count; i++) {
        ...
        vehicles.push(createVehicle(i, reg, type, use, make, model, year, Math.round(value), Math.round(basePremium), [ageOfVehicle, driverAge, location, claimsHistory, mileage, securityFeatures]));
    }
    return vehicles;
}
...
export const vehicleTestData = generateVehicles(100);
// Generate rollup from flat data
export function generateRollup(vehicles) {
    ...
    return Array.from(groups.entries()).map(([key, vList]) => {
        const [type, use] = key.split('|');
        const avgFactor = (factorKey) => vList.reduce((sum, v) => sum + v.ratingFactors[factorKey].modifier, 0) / vList.length;
        // Check if all vehicles have same modifier (can bulk edit without confusion)
        const canBulkEdit = (factorKey) => {
...
export const vehicleRollupTestData = generateRollup(vehicleTestData);
