...
export const expressionParsers = {
    ...
    nullParser: () => ({
        id: 'null',
        canParse: (_input) => false,
        parse: (_input) => ({ isParseable: false })
    }),
    /** Get a parser by id */
    get: (id) => {
        let parser = expressionParsers.nullParser();
        // Get the expression parser (default to math)
        if (typeof id !== 'string' && 'parse' in id) {
            parser = id;
        }
        else if (typeof id === 'string') {
            const maybeParser = parserRegistry.get(id);
            if (maybeParser) {
                parser = maybeParser;
            }
            else {
    ...
    list: () => Array.from(parserRegistry.keys())
};
// ============================================================================
 ...
 */
function looksLikeMathExpression(input) {
...
export const mathExpressionParser = {
    id: 'math',
    canParse: (input) => {
...
function looksLikeDateExpression(input) {
    const trimmed = input.trim().toLowerCase();
    // Check keywords
    if (DATE_KEYWORDS.includes(trimmed))
        return true;
    // Check patterns
    return RELATIVE_PATTERNS.some(pattern => pattern.test(trimmed));
}
function parseBaseDate(dateStr) {
...
function getLastDayOfWeek(dayName) {
    const days = ['sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday'];
    const targetDay = days.indexOf(dayName.toLowerCase());
    if (targetDay === -1)
        return new Date();
...
export const dateExpressionParser = {
    id: 'date',
    canParse: (input) => {
...
//# sourceMappingURL=expressionParsers.js.map