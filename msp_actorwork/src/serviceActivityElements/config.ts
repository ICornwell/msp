...
export function resolveConfig(): Partial<Config> {
    ...
    return {
        ...
        myUrl: SharedConfig?.getHostUrl?.(thisProduct.name, thisProduct.domain, thisProduct.version, thisProduct.variantName) || 'http://localhost:4003',
        myMFUrl: SharedConfig?.getMFHostUrl?.(thisProduct.name, thisProduct.domain, thisProduct.version, thisProduct.variantName) || 'http://localhost:3003',
        myPort: SharedConfig?.getPort?.(thisProduct.name, thisProduct.domain, thisProduct.version, thisProduct.variantName)?.toString() || '4003',
        jwtValidation: {
