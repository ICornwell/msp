...
export async function registerWithRetry(intervalMs = 5000) {
...
export async function registerUiFeatures() {
    const manifest = createActorworkManifest(config);
    const uiFeatures = (manifest.services || []).flatMap((service) => (service.uiFeatures || []).map((feature) => ({
        ...feature,
...
//# sourceMappingURL=manifestRegistration.js.map